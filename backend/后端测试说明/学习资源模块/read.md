...
系统启动时 **自动检测** 数据库中学习资源的数量：
- 如果 **少于 100 条**，会自动调用 AI 分 4 轮生成覆盖 **12 个领域**（前端/后端/数据库/移动开发/AI/数据科学/DevOps/云计算/网络安全/UI设计/项目管理/软件测试）的学习资源并持久化到数据库
- 如果已达到 100 条以上，则跳过播种
- 播种在启动后约 **10 秒** 开始执行，整个过程可能耗时 1-3 分钟
- 生成的资源均标记为 `aiGenerated: true`

> **测试时无需预先准备数据**，启动后端等待播种完成后即可直接测试各接口。
